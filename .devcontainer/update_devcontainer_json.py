...
first and only the keys managed by this script are rewritten. Everything else (custom environment entries, settings, mounts, and features)
is kept verbatim, so re-running the configure script never wipes project-specific
...
def main() -> int:
    ...
    data.setdefault("name", "MathCore C++/CUDA")

    # Managed: Dockerfile selection; project-specific build arguments survive.
