...
is built from .devcontainer/Dockerfile when missing or when --build is given.
CUDA and GPU access are enabled only with --gpu. --vscode instead starts a
detached attachment container.

Options:
  --build              Force (re)build of the image.
  --gpu                Install CUDA and request GPU access.
  --no-gpu             Keep the CPU-only default.
...
if [[ "$need_build_" == "yes" ]]; then
  ...
  echo "Building ${IMAGE_TAG} with ${ENGINE} (INSTALL_CUDA=${install_cuda_})..."
  "$ENGINE" build \
    --build-arg INSTALL_CUDA="$install_cuda_" \
